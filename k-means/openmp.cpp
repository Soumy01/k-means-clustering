...
bool update_clusters(vector<Cluster> &clusters); v
oid draw_chart_gnu(vector<Point> &points);
int main() { 
	printf("Number of points %d\n", num_point); printf("Number of clusters %d\n", num_cluster);
	printf("Number of processors: %d\n", omp_get_num_procs()); 
	srand(int(time(NULL)));
	double time_point1 = omp_get_wtime(); 
	printf("Starting initialization..\n");
	vector<Point> points; vector<Cluster> clusters;
//The Cluster Initialization and the Point initialization are in parallel #pragma omp parallel 
	...
	{ 
		...
		{ 
			#pragma omp section 
			{ 
				printf("Creating points..\n"); points = init_point(num_point); printf("Points initialized \n");
			} 
			#pragma omp section 
			{ 
				printf("Creating clusters..\n"); clusters = init_cluster(num_cluster); printf("Clusters initialized \n");
			} 
		} 
	} 
...
void compute_distance(vector<Point> &points, vector<Cluster> &clusters){ 
	...
	int min_index;
	#pragma omp 
	parallel default(shared) private(min_distance, min_index) 
...
double euclidean_dist(Point point, Cluster cluster){ 
	double distance = sqrt(pow(point.get_x_coord() - +pow(point.get_y_coord() - cluster.get_y_coord(),2)); 
	return distance;
} 
